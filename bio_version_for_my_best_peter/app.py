...
def infer_source(row: pd.Series) -> str:
    s = str(row.get("source", "") or "").strip().lower()
    if s:
        return s
...
def paper_to_row_bio(p: Any) -> Dict[str, Any]:
    return {
        "uid": p.uid,
        "source": p.source,
...
# Streamlit UI
# ----------------------------
st.set_page_config(page_title="Bio Paper Recommender (Browse + Match)", layout="wide")
st.title("🧬 Bio Paper Recommender（PubMed / EuropePMC / bioRxiv / medRxiv + Upload + Match）")
st.caption("Quality 建议用 run_ingest_quality.py 跑；本 app 也支持上传后立即 Quality（可选）。")
...
use_llm = st.sidebar.checkbox("启用 GPT（用于 Quality/Match）", value=True)
...
tab_browse, tab_upload, tab_bio, tab_match = st.tabs([
    "👀 浏览（Quality/Match结果）",
    "⬆️ 批量上传 PDF",
    "🧬 抓取 Bio Sources",
...
with tab_browse:
    ...
    if len(df_lib) == 0:
        st.info("你的库还是空的：先去『抓取 Bio Sources』或『上传 PDF』。")
    else:
        ...
        with colD:
            source_filter = st.selectbox("来源", ["全部", "pubmed", "europe_pmc", "biorxiv", "medrxiv", "local"], index=0)
        with colE:
        ...
        if keyword.strip():
            ...
            def _hit(r: pd.Series) -> bool:
                fields = ["title", "abstract", "quality_reason", "match_reason", "match_summary"]
                for f in fields:
        ...
        if len(pick) > 0:
            ...
            st.write(f"**Source URL**: {r.get('source_url')}")
            st.write(f"**PDF URL**: {r.get('pdf_url')}")
            st.write(f"**Local PDF**: {r.get('pdf_path')}")
            st.write(f"**DOI**: {r.get('doi')}")
            st.write(f"**PMID**: {r.get('pmid')}")
            st.write(f"**Journal**: {r.get('journal')}")
            st.markdown("---")
            st.markdown("#### ⭐ Quality")
...
# ----------------------------
with tab_bio:
    st.subheader("🧬 抓取生物论文（PubMed / EuropePMC / bioRxiv / medRxiv）")

    sources = st.multiselect(
        "数据源",
        options=["pubmed", "europe_pmc", "biorxiv", "medrxiv"],
        default=["pubmed", "europe_pmc", "biorxiv"],
    )

    query = st.text_input(
        "搜索 query（PubMed/Europe PMC 会使用这个搜索）",
        value='("single cell" OR scRNA-seq OR "single-cell") AND (genomics OR transcriptome)',
    )

    keywords_filter = st.text_input(
        "bioRxiv/medRxiv 关键词过滤（逗号分隔，可留空）",
        value="single cell, scRNA, genomics",
    )

    last_days = st.slider("只抓最近 N 天", 1, 60, 14)
    fetch_n = st.slider("本次写入数量（大概）", 10, 500, 150, step=10)

    biorxiv_category = st.text_input("bioRxiv category（可选）", value="cell_biology")
    medrxiv_category = st.text_input("medRxiv category（可选）", value="")
    ...
    if run_fetch:
        ...
        kw_list = [x.strip() for x in keywords_filter.split(",") if x.strip()]
        ...
        with st.status("抓取中...", expanded=True) as status:
            ...
            while got < fetch_n:
                papers, state = fetch_bio_batch(
                    sources=sources,
                    query=query,
                    page_size=min(80, fetch_n - got),
                    last_days=last_days,
                    state=state,
                    keywords_filter=kw_list if kw_list else None,
                    biorxiv_category=biorxiv_category.strip() or None,
                    medrxiv_category=medrxiv_category.strip() or None,
                )
                ...
                for p in papers:
                    rows.append(paper_to_row_bio(p))
            ...
            if rows:
                ...
                status.update(label="⚠️ 没抓到任何论文（可能 query 太窄/时间窗口太小）", state="complete")
...
with tab_match:
    ...
    st.caption("本页做 Match：调用 llm_match.match_one 写回 match_*。")

    df_now = load_csv(csv_path)
    if len(df_now) == 0:
        st.info("库为空。先抓 Bio Sources 或上传 PDF。")
    else:
        ...
        with colD:
            min_match_download = st.slider("match_score ≥ X 才自动下载 PDF（有 pdf_url 才会下载）", 0, 100, 85)
        with colE:
            per_call_sleep = st.slider("每次调用间隔（秒，防限速）", 0.0, 1.0, 0.0, step=0.1)

        prompt = st.text_area(
            "你的需求（prompt）",
            value="我想找关于 single-cell / cell atlas / multi-omics 的最新高质量论文，偏方法论与可复现，最好有代码。",
            height=140,
        )
        ...
        if run_match:
            ...
            # 自动下载：按 match_score（只对有 pdf_url 的条目）
            if download_top_k > 0:
                ...
                df_dl = df_after[
                    ...
                    & (df_after["pdf_url"].fillna("").astype(str).str.startswith("http"))
                ].copy()
